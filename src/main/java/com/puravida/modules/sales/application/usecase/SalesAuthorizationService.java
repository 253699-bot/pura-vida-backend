...
@Service
public class SalesAuthorizationService {

    private final UserRepositoryPort userRepositoryPort;

    public SalesAuthorizationService(UserRepositoryPort userRepositoryPort) {
    ...
    public User requireEncargada(AuthenticatedUser authenticatedUser) {
        ...
        User actor = userRepositoryPort.findById(authenticatedUser.userId())
                .orElseThrow(() -> new ForbiddenException("No tienes permisos para administrar ventas."));

        if (!actor.activo() || actor.rol() != UserRole.ENCARGADA) {
            throw new ForbiddenException("No tienes permisos para administrar ventas.");
        }
