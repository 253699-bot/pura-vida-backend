...
public class DashboardAuthorizationService {
    ...
    public DashboardAuthorizationService(UserRepositoryPort userRepositoryPort) {
    ...
    public User requireEncargada(AuthenticatedUser authenticatedUser) {
        ...
        User actor = userRepositoryPort.findById(authenticatedUser.userId())
                .orElseThrow(() -> new ForbiddenException("No tienes permisos para consultar el dashboard."));
        if (!actor.activo() || actor.rol() != UserRole.ENCARGADA) {
            throw new ForbiddenException("No tienes permisos para consultar el dashboard.");
        }
        return actor;
    }
}
